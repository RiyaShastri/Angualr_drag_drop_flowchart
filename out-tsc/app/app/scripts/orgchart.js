...
var OrgChartUtilityMethods = (function () {
    ...
    OrgChartUtilityMethods.readFile = function (event) {
        ...
        if (this.fileType === 'csv') {
            this.orgDataSource = OrgChartUtilityMethods.convertCsvToJson(event.target.result.toString());
        }
        else if (this.fileType === 'json') {
            this.orgDataSource = JSON.parse(event.target.result.toString());
            ...
            var xmlDom = parser.parseFromString(event.target.result.toString(), 'text/xml');
    ...
    OrgChartUtilityMethods.removeChild = function () {
        var diagram = this.selectedItem.selectedDiagram;
        if (diagram.selectedItems.nodes.length > 0) {
            this.selectedItem.preventPropertyChange = true;
    ...
    OrgChartUtilityMethods.removeSubChild = function (node) {
        var diagram = this.selectedItem.selectedDiagram;
    ...
    OrgChartUtilityMethods.addChild = function (sourceId) {
        ...
        for (var i = 0; i < keys.length; i++) {
            var key = keys[i];
            var keyValue = parentNode.addInfo[key];
    ...
    OrgChartUtilityMethods.bindFields = function () {
        ...
        for (var i = 0; i < this.orgDataSource.length; i++) {
            ...
            this.orgDataSource[i].addInfo = cloneObject(addInfo);
        }
    ...
    OrgChartUtilityMethods.applyDataSource = function () {
        ...
        var items = new DataManager(this.bindFields());
