...
export class HomeComponent implements AfterViewInit {
    ...
    @ViewChild('ejDialogLocal') ejDialogLocal: DialogComponent;

    drive = false;
    driveForLocal = false;
    googleAuth: gapi.auth2.GoogleAuth;
    Drivedata: any = [];
    currentFile: any = "Untitled Diagram";
    ...
    public getDialogButtons(dialogType: string): Object[] {
        ...
        switch (dialogType) {
            ...
            // case 'saveasOnLocal':
            //     buttons.push({
            //         click: this.btnLocalAs.bind(this)
            //     });
            //     break;
            case 'tooltip':
                buttons.push({
    ...
    private btnLocal(): void {
        const data = document.getElementById('saveLocal');
        const d = (document.getElementById('saveLocal') as HTMLInputElement).value;
        CommonKeyboardCommands.download(this.page.savePage(d), d);
        this.currentFile = d + '.json';
        this.driveForLocal = false;
    }
    private btnCancelLocal(): void {
        this.driveForLocal = false;
    }

    // private btnLocalAs(): void {

    //     let d = (document.getElementById('saveAsLocal') as HTMLInputElement).value;
    //     console.log(d);

    //     // // CommonKeyboardCommands.download(this.page.savePage(), d);
    //     // CommonKeyboardCommands.download(this.page.savePage(), d);
    //     this.currentFile = d;
    //     CommonKeyboardCommands.download(this.page.savePage(d), d);
    //     this.drive1 = false;
    // }
    // private btnCancelLocalAs(): void {
    //     this.drive1 = false;
    // }
    ...
    public opendfilefromdrive(data: string) {
        const accessToken = gapi.auth.getToken().access_token;
        const xhr = new XMLHttpRequest();
        xhr.open('get', 'https://drive.google.com/file/d/1XwAfvBRz7kZzRSZfCprrX7ZL_4w5STg-/preview');
        xhr.setRequestHeader('Authorization', 'Bearer ' + accessToken);
        xhr.responseType = 'json';
        xhr.onload = () => {
            this.drive = true;
            this.Drivedata = xhr.response;
    ...
    signInFordrive() {
        ...
        const check = this.googleAuth.isSignedIn.get();
    ...
    public GetDrivedata() {
        const accessToken = gapi.auth.getToken().access_token;
        const xhr = new XMLHttpRequest();
        xhr.open('get', 'https://www.googleapis.com/drive/v2/files?key=AIzaSyDBg6zSDCn6o0XSoiA2nIUyPg6THLzp9T4&trashed=true');
        xhr.setRequestHeader('Authorization', 'Bearer ' + accessToken);
        xhr.responseType = 'json';
        xhr.onload = () => {
            this.drive = true;
            this.Drivedata = xhr.response;
            this.Drivedata = xhr.response.items.filter(x => x.mimeType === 'application/json');
        }
        xhr.send();
    }
    ...
    filepreview(id, file) {
        ...
        this.currentFile = file + '.json';
        const drivefile: PageCreation = new PageCreation(this.selectedItem);

        const accessToken = gapi.auth.getToken().access_token;
        console.log(accessToken);

        const i = this.Drivedata.filter(x => x.id === id);
        // console.log(i);

        if (i[0].downloadUrl) {
            const url = i[0].downloadUrl;
            var xhr = new XMLHttpRequest();
            xhr.open('GET', url);
            xhr.setRequestHeader('Authorization', 'Bearer ' + accessToken);
            xhr.onload = function () {
                console.log(xhr.responseText);
                drivefile.loadPage(xhr.responseText);
                drivefile.loadDiagramSettings();
            };
            xhr.send();
    ...
    public getfile() {
        var filename = (document.getElementById('saveFileName') as HTMLInputElement).value;
        const accessToken = gapi.auth.getToken().access_token;
        const xhr = new XMLHttpRequest();
        xhr.open('get', 'https://www.googleapis.com/drive/v2/files?key=AIzaSyDBg6zSDCn6o0XSoiA2nIUyPg6THLzp9T4&trashed=true');
        console.log(accessToken);
        ...
        xhr.responseType = 'json';
        xhr.onload = () => {
            console.log('Get All File!', xhr.response);
            this.post = xhr.response;
            let isFound = false;
            let id = null;
            for (let i = 0; i < this.post.items.length; i++) {
                if (this.post.items[i].title === filename && this.post.items[i].labels.trashed == false) {
                    console.log(this.post.items[i].title);
                    isFound = true;
                    id = this.post.items[i].id;
                }
            }
    ...
    public filedelete(id) {
        const accessToken = gapi.auth.getToken().access_token;
        const xhr = new XMLHttpRequest();
        xhr.open('DELETE', 'https://www.googleapis.com/drive/v3/files/' + id);
        xhr.setRequestHeader('Authorization', 'Bearer ' + accessToken);
        xhr.responseType = 'json';
        xhr.onload = () => {
            console.log("Deleted Old"); // Retrieve uploaded file ID.
        };
        xhr.send();
    }
    ...
    public upload() {
        var filename = (document.getElementById('saveFileName') as HTMLInputElement).value;
        let dataStr = CommonKeyboardCommands.page.savePage(filename);// As a sample, upload a text file.
    ...
    public menuClick(args: MenuEventArgs): void {
        ...
        switch (commandType.toLowerCase()) {
            case 'new':
                ...
                this.currentFile = 'Untitled Diagram';
                break;
            ...
            case 'openwithdrive':
                this.initClientForDrive();
                break;

            case 'save':
                this.driveForLocal = true;
                this.ejDialogLocal.show();
            ...
            case 'fittoscreen':
                diagram.fitToPage({ mode: 'Page', region: 'Content', margin: { left: 20, top: 20, right: 20, bottom: 20 } });
                break;
    ...
    private generateDiagram(): void {
        let diagram: CustomDiagram = new CustomDiagram({
            ...
            pageSettings: {
                background: { color: 'white' }, width: 100, height: 100, multiplePage: true, margin: { left: 5, top: 5 },
    ...
    public onUploadSuccess(args: { [key: string]: Object }): void {
        this.currentFile = args.file['name'];
    ...
    public setImage(event: ProgressEvent): void {
        //(document.getElementsByClassName('sb-content-overlay')[0] as HTMLDivElement).style.display = 'none';
        let node: any = this.selectedItem.selectedDiagram.selectedItems.nodes[0];
