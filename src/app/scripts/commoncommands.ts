...
import { PageOptions, PageCreation } from '../scripts/pages';
...
export abstract class CommonKeyboardCommands {
 ...
 currentFile : string;
    ...
    // public newselectedItem: SelectorViewModel = new SelectorViewModel();
    // public static abc: PageCreation = new PageCreation(this.selectedItem);
    ...
    public static saveDiagram(): void {
        this.download(this.page.savePage(document.getElementById('diagramName').innerHTML), document.getElementById('diagramName').innerHTML);
    }
    ...
    public static download(data: string, filename: string): void {
        ...
        parsedata.pageOptionList[0].name = filename;
        dataStr = JSON.stringify(parsedata);
        if (window.navigator.msSaveBlob) {
            let blob: Blob = new Blob([dataStr], { type: ' ,' });
